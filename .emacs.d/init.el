    ...
    ((rspec-use-bundler-when-possible)
           ...
           (lambda
             ...
             (progn
               ...
               (print
                (append
                 (quote
                  ("docker-compose" "exec" "spring" "bundle" "exec"))
                 (last
                  (split-string
                   (car command)
                   "/"))
               ...
               (append
                ...
                (last
                 (split-string
                  ...
                  "/"))
                (cdr command)))))
     (flycheck-phpstan-executable quote nil)
     (eval setq flycheck-command-wrapper-function
           (lambda
             (command)
             (progn
     ...
     (eval setq flycheck-command-wrapper-function
           (lambda
             ...
             (progn
               (print
                (shell-command-to-string "docker container ls"))
               (append
                (quote
                 ("docker-compose" "exec" "spring" "bundle" "exec"))
                command))))
             ...
             (progn
               ...
               (append
                (quote
                 ("docker-compose" "exec" "spring" "bundle" "exec"))
                command))))
     (setq rubocop-check-command "docker-compose exec spring bundle exec rubocop")
     (setq rubocop-check-command "docker-compOse exec spring bundle exec rubocop")
     (eval setq flycheck-command-wrapper-function
           (lambda
             (command)
